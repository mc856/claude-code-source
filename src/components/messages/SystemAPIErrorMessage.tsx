...
export function SystemAPIErrorMessage(t0: Props) {
  ...
  if ($[19] !== maxRetries || $[20] !== retryAttempt || $[21] !== retryInSecondsLive || $[22] !== t9) {
    t10 = <Text dimColor={true}>Retrying in {retryInSecondsLive}{" "}{t9}â€?(attempt{" "}{retryAttempt}/{maxRetries}){process.env.API_TIMEOUT_MS ? ` Â· API_TIMEOUT_MS=${process.env.API_TIMEOUT_MS}ms, try increasing it` : ""}</Text>;
    $[19] = maxRetries;
    $[20] = retryAttempt;
