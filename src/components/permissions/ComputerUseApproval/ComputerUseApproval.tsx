...
function ComputerUseAppListPanel(t0: ComputerUseApprovalProps) {
  const $ = _c(48);
  const {
    request,
    onDone
  ...
  if ($[3] !== request.requestedFlags) {
    t3 = ALL_FLAG_KEYS.filter((k: keyof typeof DEFAULT_GRANT_FLAGS) => request.requestedFlags[k]);
...
function _temp4(flag: keyof typeof DEFAULT_GRANT_FLAGS) {
  return <Text key={flag} dimColor={true}>{"  "}· {flag}</Text>;
}
function _temp3(k_0: keyof typeof DEFAULT_GRANT_FLAGS) {
