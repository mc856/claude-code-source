...
export function PermissionDecisionDebugInfo(t0: Props) {
  ...
  const message = 'message' in permissionResult ? permissionResult.message : undefined;
  let t1;
  ...
  if ($[9] !== permissionResult.behavior || $[10] !== message) {
    t4 = permissionResult.behavior !== "allow" && message !== undefined && <Box flexDirection="row"><Box justifyContent="flex-end" minWidth={10}><Text dimColor={true}>Message </Text></Box><Text>{message}</Text></Box>;
...
function _temp4(s: { toolPermissionContext: any }) {
