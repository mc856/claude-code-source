...
import { getAgentMemoryDir, type AgentMemoryScope } from '../../tools/AgentTool/agentMemory.js';
...
export function MemoryFileSelector(t0: Props) {
  ...
  const activeTeamMemPaths = teamMemPaths;
  if (isAutoMemoryEnabled()) {
    ...
    if (feature("TEAMMEM") && activeTeamMemPaths?.isTeamMemoryEnabled()) {
      let t2;
      if ($[1] === Symbol.for("react.memo_cache_sentinel")) {
        t2 = {
          label: "Open team memory folder",
          value: `${OPEN_FOLDER_PREFIX}${activeTeamMemPaths.getTeamMemPath()}`,
          description: ""
        };
        $[1] = t2;
      } else {
    ...
    for (const agent of agentDefinitions.activeAgents) {
      if (agent.memory) {
        const agentDir = getAgentMemoryDir(agent.agentType, agent.memory as AgentMemoryScope);
        folderOptions.push({
