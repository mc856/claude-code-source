...
const MessagesImpl = ({
  ...
  const {
    ...
    const messagesToShowNotTruncated: NonProgressMessage[] = reorderMessagesInUI(compactAwareMessages.filter((msg_2): msg_2 is NonProgressMessage => msg_2.type !== 'progress')
    ...
    const briefFiltered: NonProgressMessage[] = briefToolNames.length > 0 && !isTranscriptMode ? isBriefOnly ? filterForBriefTool(messagesToShowNotTruncated, briefToolNames) : dropTextToolNames.length > 0 ? dropTextInBriefTurns(messagesToShowNotTruncated, dropTextToolNames) : messagesToShowNotTruncated : messagesToShowNotTruncated;
    const messagesToShow: NonProgressMessage[] = shouldTruncate ? briefFiltered.slice(-MAX_MESSAGES_TO_SHOW_IN_TRANSCRIPT_MODE) : briefFiltered;
  ...
  const extractSearchText = useCallback((msg_9: RenderableMessage): string => {
    ...
    if (msg_9.type === 'user' && msg_9.toolUseResult && Array.isArray(msg_9.message.content)) {
      const tr = msg_9.message.content.find((b_1: {
        type: string;
        tool_use_id?: string;
      }) => b_1.type === 'tool_result');
...
export function shouldRenderStatically(message: RenderableMessage, streamingToolUseIDs: Set<string>, inProgressToolUseIDs: Set<string>, siblingToolUseIDs: ReadonlySet<string>, screen: Screen, lookups: ReturnType<typeof buildMessageLookups>): boolean {
  ...
  switch (message.type) {
    ...
    case 'grouped_tool_use':
      {
        const allResolved = message.messages.every((msg: typeof message.messages[number]) => {
