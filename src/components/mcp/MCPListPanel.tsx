...
export function MCPListPanel(t0: Props) {
  ...
  if ($[48] !== renderServerItem || $[49] !== serversByScope) {
    t22 = SCOPE_ORDER.map((scope_0: ConfigScope) => {
  ...
  if ($[54] !== agentServers || $[55] !== renderAgentServerItem) {
    const sourceAgentNames: string[] = [...new Set<string>(agentServers.flatMap(_temp6))];
    t24 = agentServers.length > 0 && <Box flexDirection="column" marginBottom={1}><Box paddingLeft={2}><Text bold={true}>Agent MCPs</Text></Box>{sourceAgentNames.map((agentName: string) => <Box key={agentName} flexDirection="column" marginTop={1}><Box paddingLeft={2}><Text dimColor={true}>@{agentName}</Text></Box>{agentServers.filter((s_3: AgentMcpServerInfo) => s_3.sourceAgents.includes(agentName)).map((agentServer_2: AgentMcpServerInfo) => renderAgentServerItem(agentServer_2))}</Box>)}</Box>;
    $[54] = agentServers;
