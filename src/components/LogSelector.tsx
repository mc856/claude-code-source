...
export function LogSelector(t0: LogSelectorProps) {
  ...
  const [deepSearchResults, setDeepSearchResults] = React.useState<DeepSearchResultsState>(null);
  ...
  bb3: {
    ...
    if ($[73] !== displayedLogs || $[74] !== highlightColor || $[75] !== maxLabelWidth || $[76] !== showAllProjects || $[77] !== snippets) {
      ...
      if ($[79] !== highlightColor || $[80] !== maxLabelWidth || $[81] !== showAllProjects || $[82] !== snippets) {
        t32 = (log_9: LogOption, index_0: number): FlatOption => {
  ...
  if ($[94] !== logs || $[95] !== onAgenticSearch || $[96] !== searchQuery) {
    t35 = async () => {
      ...
      ;
      try {
        const agenticSearch = onAgenticSearch;
        if (!agenticSearch) {
          return;
        }
        const results_0 = await agenticSearch!(searchQuery, logs, abortController.signal);
        ...
        let message: string = "Search failed";
        if (typeof t36 === "object" && t36 !== null) {
          const maybeMessage = (t36 as {
            message?: unknown;
          }).message;
  ...
  if ($[131] !== agenticSearchState.status || $[132] !== branchFilterEnabled || $[133] !== focusedLog || $[134] !== handleAgenticSearch || $[135] !== hasMultipleWorktrees || $[136] !== hasTags || $[137] !== isAgenticSearchOptionFocused || $[138] !== onAgenticSearch || $[139] !== onToggleAllProjects || $[140] !== searchQuery || $[141] !== setSearchQuery || $[142] !== showAllProjects || $[143] !== showAllWorktrees || $[144] !== tagTabs || $[145] !== uniqueTags || $[146] !== viewMode) {
    t53 = (input: string, key: { ctrl?: boolean; return?: boolean; downArrow?: boolean; upArrow?: boolean; escape?: boolean; tab?: boolean; shift?: boolean; meta?: boolean }) => {
      if (viewMode === "preview") {
        return;
      }
...
function _temp5(fuseIndex_0: any, debouncedDeepSearchQuery_0: string, setDeepSearchResults_0: React.Dispatch<React.SetStateAction<DeepSearchResultsState>>, setIsSearching_0: React.Dispatch<React.SetStateAction<boolean>>) {
...
function _temp4(r: any): DeepSearchResult {
  return {
    log: r.item.log,
    score: r.score,
