...
function printMissingImports(missingImports: MissingImport[]): void {
  ...
  for (const item of missingImports.slice(0, 20)) {
    console.log(
      `- ${item.importer.replace(`${process.cwd()}\\`, '').replace(`${process.cwd()}/`, '')} -> ${item.specifier}`,
    )
  }
}
...
if (args.includes('--help')) {
  ...
  console.log('')
  console.log(
    'Interactive REPL startup is routed to src/entrypoints/cli.tsx when restored imports are complete.',
  )
  process.exit(0)
}

if (missingImports.length > 0) {
  printMissingImports(missingImports)
  console.log('')
  console.log('The original app entry is still blocked by missing restored sources.')
  console.log(
    'Use this workspace to continue restoration; once missing imports reach 0, this launcher will forward to src/entrypoints/cli.tsx automatically.',
  )
  process.exit(0)
}
