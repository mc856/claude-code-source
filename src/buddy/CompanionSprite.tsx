...
export function CompanionSprite(): React.ReactNode {
  const reaction = useAppState((s: AppState) => s.companionReaction) as
    | string
    | undefined;
  const petAt = useAppState((s: AppState) => s.companionPetAt) as
    | number
    | undefined;
  const focused = useAppState(
    (s: AppState) => s.footerSelection === 'companion',
  ) as boolean;
...
function _temp3(set: React.Dispatch<React.SetStateAction<FloatingBubbleState>>) {
  return set(_temp2);
}
function _temp2(s_0: FloatingBubbleState): FloatingBubbleState {
  return {
    ...s_0,
    tick: s_0.tick + 1
  };
