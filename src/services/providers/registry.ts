 ...
 *   1. CLAUDE_CODE_PROVIDER env var  ("claude" | "openai" | "azure-openai")
 *   2. Legacy Anthropic-compatible env vars (CLAUDE_CODE_USE_BEDROCK etc.) → "claude"
 *   3. Default: "claude"
 *
 ...
 *
 * When CLAUDE_CODE_PROVIDER is "claude" (or unset), it delegates to the
 * existing `claudeAdapter` → `queryModelWithStreaming` with zero behaviour
 * change. For "openai" or "azure-openai" it routes to the respective adapter.
 */
